...
async function main() {
  try {
    ...
    // If there are no users in the app DB, create a safe fallback test user so
    // the rest of the seed can assign authorId values. You can override the
    // ID and email with DEV_SEED_USER_ID / DEV_SEED_USER_EMAIL env vars.
    if (users.length === 0) {
      const fallbackId = process.env.DEV_SEED_USER_ID ?? "seed-user-1";
      const fallbackEmail =
        process.env.DEV_SEED_USER_EMAIL ?? "seed@example.com";

      console.warn(
        `⚠️ No users found: inserting fallback test user (${fallbackId}, ${fallbackEmail})`
      );

      await sql.query(
        `INSERT INTO neon_auth.users_sync (id, email, created_at) VALUES ($1, $2, now()) ON CONFLICT (id) DO NOTHING`,
        [fallbackId, fallbackEmail]
      );

      // re-query users after inserting fallback
      users = await db
        .select({ id: usersSync.id })
        .from(usersSync)
        .orderBy(usersSync.id);

      console.log(`👥 Found ${users.length} user(s) after fallback insert`);
    }
